...
def get_all_oe_dataset(args, need_str_enc=True,max_l=-1):
    print(f"get_all_oe_dataset: {args.DS} {args.DS_ood}")
    if args.DS_pair is not None:
        DSS = args.DS_pair.split("+")
        DS, DS_ood = DSS[0], DSS[1]
    else:
...
def get_ood_dataset_testtime(args, train_per=0.9, need_str_enc=True):
    if args.DS_pair is not None:
        DSS = args.DS_pair.split("+")
        DS, DS_ood = DSS[0], DSS[1]
        args.DS=DS
    ...
    count=min(len(dataset),len(dataset_ood))

    dataset_train = dataset[:count]
    dataset_test = dataset[:count]
    dataset_ood = dataset_ood[:count]
...
def get_ood_dataset_new(args, train_per=0.5, need_str_enc=True,shuffle=True):
    if args.DS_pair is not None:
        DSS = args.DS_pair.split("+")
        DS, DS_ood = DSS[0], DSS[1]
        args.DS=DS
    ...
    if need_str_enc:
        data_list_train = init_structural_encoding(data_list_train, rw_dim=args.rw_dim, dg_dim=args.dg_dim)
    dataloader = DataLoader(data_list_train, batch_size=args.batch_size, shuffle=shuffle)
    ...
    if train_per == 1.0:
        ...
        dataloader_test = DataLoader(data_list_test, batch_size=args.batch_size, shuffle=shuffle)
...
def get_ood_dataset_new_diverse(args, DS='PTC_MR', DS_ood='MUTAG', train_per=0.5, need_str_enc=True,shuffle=True):
    # DS, DS_ood = DS, DS_ood
